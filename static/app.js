...
function renderPrice(data) {
    try {
        ...
        console.error("[V16] Fallo en renderPrice:", e);
    }
}
...
function renderStatus(data) {
    try {
        ...
        console.error("[V16] Fallo en renderStatus:", e);
    }
}
...
function renderTrend(data) {
    try {
        ...
        console.error("[V16] Fallo en renderTrend:", e);
    }
}

function renderAudit(data) {
    try {
        const metrics = data.evaluacion;
        if (!metrics) return;
        ...
        ['1h', '2h', '4h', '24h'].forEach(hz => {
            const mData = metrics[hz];
            const badge = document.getElementById(`badge_${hz}`);
            const mae = document.getElementById(`mae_${hz}`);

            if (mData && mData.status === "ready") {
                if (mData.color === "green") {
                    badge.className = "audit-badge win";
                    badge.innerText = `+ GANA (${mData.accuracy}%)`;
                    mae.innerHTML = `<span class="text-success">${mData.mae_ai}%</span> vs ${mData.mae_base}%`;
                } else {
                    badge.className = "audit-badge lose";
                    badge.innerText = `- PIERDE (${mData.accuracy}%)`;
                    mae.innerHTML = `<span class="text-danger">${mData.mae_ai}%</span> vs ${mData.mae_base}%`;
                }
            } else {
        ...
        console.error("[V16] Fallo en renderAudit:", e);
    }
}
...
function renderChart(data) {
    try {
        ...
        if (!chartData || !chartData.history || !chartData.history.prices || chartData.history.prices.length === 0) {
            console.warn("[V16 Chart] Sin histórico válido. Ignorando canvas.");
            return;
        }
        ...
        if (pred && pred.prices && pred.prices.length > 0 && pred.times && pred.times.length > 0) {
            ...
            for (let i = 0; i < pred.prices.length; i++) {
                if (i < pred.times.length) {
                    ...
                    const b_pct = (pred.bounds_pct && pred.bounds_pct[i]) ? pred.bounds_pct[i] : 0.005;
        ...
        // Bloqueo del Escalamiento para la proporción 168h / 24h
        ...
        console.error("[V16] Fallo en renderChart:", e);
    }
}
...
async function fetchData() {
    try {
        ...
        if (response.ok) {
            ...
            console.log("[V16 JSON Payload Recibido]", data);

            // Degradación Elegante Extrema: renderizamos bloque por bloque independientemente
            renderPrice(data);
            renderStatus(data);
            renderTrend(data);
            renderAudit(data);
            ...
            console.error("[V16 API] HTTP Error:", response.status);
        ...
        console.error("[V16 Network] Fetch fallido:", error);
...
document.addEventListener('DOMContentLoaded', () => {
    console.log("[V16] Inicializando aplicación. Bloques de Try/Catch independientes.");
    initChart();
    fetchData();
